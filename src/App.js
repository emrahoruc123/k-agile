...
const SortableCard = ({ id, card, onDelete, onEdit }) => {
  ...
  return (
    ...
    >
      {/* BUTON PANELİ: Her zaman görünür ve yan yana */}
      <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
        <button 
          onClick={(e) => { e.stopPropagation(); onEdit(card); }}
          className="p-1.5 bg-slate-50 hover:bg-blue-50 text-slate-400 hover:text-blue-600 rounded-lg transition-colors border border-slate-100"
          title="Düzenle"
        >
          <Edit3 size={14} />
        </button>
      ...
      <div className="flex justify-between items-center pt-3 border-t border-slate-50">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] font-bold text-rose-600 uppercase tracking-tighter">{card.deadline || 'Süresiz'}</span>
        </div>
        <div className="flex items-center gap-2">
            <span className="text-[9px] font-bold text-slate-400 uppercase italic">{card.logs?.length || 0} İşlem</span>
            <div className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-600 border border-white shadow-sm">
...
export default function App() {
  ...
  const handleOpenModal = (colId, card = null) => {
    ...
    if (card) {
      ...
      setEditingCard(null);
      // Placeholder'ın çalışması için değerleri tamamen boşaltıyoruz
  ...
  const handleSave = () => {
    ...
    if (editingCard) {
      setCards(prev => ({
        ...prev, 
        [editingCard.id]: { ...formData, id: editingCard.id, logs: [{text: "Düzenlendi", time: now}, ...(editingCard.logs || [])] }
      }));
    } else {
  ...
  return (
    <div className="h-screen bg-[#f8f9fb] flex flex-col overflow-hidden font-sans text-slate-900">
      ...
      <main className="flex-1 p-10 flex gap-8 overflow-x-auto items-start">
        <DndContext sensors={sensors} collisionDetection={collisionDetectionStrategy} onDragStart={(e) => setActiveId(e.active.id)} onDragOver={handleDragOver} onDragEnd={() => setActiveId(null)}>
          {Object.keys(columns).map(colId => (
            <div key={colId} className="w-[340px] shrink-0 bg-slate-200/40 rounded-[2.5rem] flex flex-col p-4 h-full border border-slate-200/50">
              <h2 className="font-black text-slate-500 text-[11px] tracking-[0.2em] uppercase mb-6 px-4 flex justify-between items-center">
                {colId} <span className="bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full text-[9px]">{columns[colId].length}</span>
              </h2>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[1000] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-3xl rounded-[3.5rem] p-12 shadow-2xl flex flex-col max-h-[85vh] border border-white/20">
            <h2 className="text-3xl font-black italic mb-10 flex items-center gap-3">
              {editingCard ? <Edit3 className="text-rose-600" /> : <Plus className="text-rose-600" />}
              {editingCard ? 'GÖREVİ DÜZENLE' : 'YENİ GÖREV'}
            </h2>
            
            <div className="flex gap-12 overflow-hidden">
                <div className="flex-1 space-y-6">
                    <div>
                      <label className="text-[10px] font-black text-slate-400 ml-4 mb-2 block uppercase tracking-widest">Görev Tanımı</label>
                      <input type="text" placeholder="Örn: Landing page tasarımlarını bitir..." value={formData.title} className="w-full p-5 bg-slate-50 rounded-[1.5rem] outline-none focus:ring-2 ring-rose-500/20 text-slate-700 font-medium transition-all" onChange={e => setFormData({...formData, title: e.target.value})}/>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                          <label className="text-[10px] font-black text-slate-400 ml-4 mb-2 block uppercase tracking-widest">Kategori</label>
                          <select value={formData.tag} className="w-full p-5 bg-slate-50 rounded-[1.5rem] outline-none font-bold text-slate-600" onChange={e => setFormData({...formData, tag: e.target.value})}>
                              <option value="GÖREV">GENEL GÖREV</option><option value="UI/UX">UI/UX TASARIM</option><option value="DEV">YAZILIM GELİŞTİRME</option><option value="TEST">QA / TEST</option>
                          </select>
                        </div>
                        <div>
                          <label className="text-[10px] font-black text-slate-400 ml-4 mb-2 block uppercase tracking-widest">Sorumlu</label>
                          <input type="text" placeholder="İsim giriniz..." value={formData.assignee} className="w-full p-5 bg-slate-50 rounded-[1.5rem] outline-none focus:ring-2 ring-rose-500/20 text-slate-700 font-medium" onChange={e => setFormData({...formData, assignee: e.target.value})}/>
                        </div>
                    </div>

                    <div>
                      <label className="text-[10px] font-black text-slate-400 ml-4 mb-2 block uppercase tracking-widest">Teslim Tarihi</label>
                      <input type="date" value={formData.deadline} className="w-full p-5 bg-slate-50 rounded-[1.5rem] outline-none text-slate-600 font-bold" onChange={e => setFormData({...formData, deadline: e.target.value})}/>
                    </div>

                    <div className="flex gap-4 pt-6">
                        <button onClick={handleSave} className="flex-[2] bg-rose-600 text-white font-black py-5 rounded-[1.5rem] shadow-xl hover:bg-rose-700 transition-all flex items-center justify-center gap-3 tracking-widest"><Save size={20}/> KAYDET</button>
                        <button onClick={() => setShowModal(false)} className="flex-1 bg-slate-100 text-slate-500 font-black py-5 rounded-[1.5rem] hover:bg-slate-200 transition-all flex items-center justify-center gap-3 tracking-widest"><Ban size={20}/> İPTAL</button>
                    </div>
                </div>
                    ...
                    <div className="w-72 border-l border-slate-100 pl-10 flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-4 pr-4 scrollbar-hide">
                            {(editingCard.logs || []).map((log, i) => (
                                <div key={i} className="text-[11px] border-l-2 border-rose-300 pl-4 py-1 relative">
                                    <div className="absolute -left-[5px] top-2 w-2 h-2 rounded-full bg-rose-400"></div>
                                    <p className="font-bold text-slate-700 leading-tight flex items-center gap-2 uppercase tracking-tighter">
                                        {log.text.includes('➔') ? <span className="flex items-center gap-1 text-[10px]"><ArrowRight size={12} className="text-rose-500" /> {log.text}</span> : log.text}
                                    </p>
                                    <span className="text-slate-400 font-bold text-[9px]">{log.time}</span>
                                </div>
                            ))}
                        </div>
                    </div>
